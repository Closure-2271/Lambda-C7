...
async function Checkpoint262(interaction) {
  const browser = await puppeteer.launch({ headless: "new" });
  const page = await browser.newPage();
  await page.goto('https://d2checkpoint.com/', { waitUntil: 'networkidle0' });

  const data = await page.evaluate(() => {
    const element = document.querySelector('[id="262"]');
...
async function checkCheckpoint262() {
  const browser = await puppeteer.launch({ headless: "new" });
  const page = await browser.newPage();
  await page.goto('https://d2checkpoint.com/', { waitUntil: 'networkidle0' });
...
module.exports = { Checkpoint262, checkCheckpoint262 };
