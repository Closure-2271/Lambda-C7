...
async function Checkpoint233(interaction) {
  const browser = await puppeteer.launch({ headless: "new" });
  const page = await browser.newPage();
  await page.goto('https://d2checkpoint.com/', { waitUntil: 'networkidle0' });

  const data = await page.evaluate(() => {
    const element = document.querySelector('[id="233"]');
...
async function checkCheckpoint233() {
  const browser = await puppeteer.launch({ headless: "new" });
  const page = await browser.newPage();
  await page.goto('https://d2checkpoint.com/', { waitUntil: 'networkidle0' });
...
module.exports = { Checkpoint233, checkCheckpoint233 };
